    ...
    if results.multi_hand_landmarks:
        for hand_landmark in results.multi_hand_landmarks:
            ...
            if all(finger_fold_status):
                # faça uma captura de tela da tela e armazene-a na memória e, em seguida,
                # converta a imagem PIL/Pillow em um array NumPy compatível com OpenCV
                # e finalmente grave a imagem no disco
                image = pyautogui.screenshot()
                image = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
                cv2.imwrite("in_memory_to_disk.png", image)

                # tirar uma captura de tela diretamente no disco
